...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/Z0fxkgAKKLMAVEVNlW7gDqZIFzSvVq29QfqhfAgO7E6Tehuq8zJhMjvO0RpvNLVsu3FMM0YFRo2WTMe3JiawQoA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/Z0fxkgAKKLMAVEVNlW7gDqZIFzSvVq29GQ8Uo1gI0b9CSqpR82q0KsouY0haBPsUGIWqWAUqjS9judjarXguVA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
3. 若Java虚拟机栈采用**不可扩展**的，超过Java虚拟机栈的存储空间后，会报**StackOverflowError** 异常

4. 若Java虚拟机栈采用**可扩展**的，超过Java虚拟机栈的存储空间后会报**OutOfMemoryError**异常
...
1. 在方法执行过程中，根据字节码指令，往操作数栈中写入数据或提取数据，即入栈和出栈。
2. 操作数栈主要用于保存计算过程的中间结果，同时作为计算过程中的变量临时的存储空间。
3. 操作数栈就是JVM执行引擎的一个工作区，当一个方法刚开始执行的时候，一个新的栈帧也会随之被创建出来，此时这个方法的操作数栈是空的。
4. 如果被调用的方法带有返回值，其返回值会被压入当前栈帧的操作数栈中，并更新PC寄存器中下一条需要执行的字节码指令。

#### C. 动态连接

1. 每一个栈帧内部都包含一个只想运行时常量池中该栈所属方法的引用。

2. 作用为将在Java源文件被编译到字节码文件中时，所有变量和方法引用为符号引用，将此时的符号引用转换为调用方法的直接引用。
...
1. 用来存储该方法的PC寄存器的值
2. 执行引擎遇到任意一个方法返回的字节码指令，会有返回值传递给上层的方法调用者，简称**正常完成出口**
3. 在方法执行的过程中遇到了异常，并且这个异常没有在方法内进行处理，也就是只要在本方法的异常表中没有搜索到匹配的异常处理器，就会导致方法退出。简称**异常完成出口**
...
1. 线程私有
2. 本地方法是使用C语言实现的
3. 它的具体做法是 `Mative Method Stack` 中登记native方法，在 `Execution Engine` 执行时加载本地方法库当某个线程调用一个本地方法时，它就进入了一个全新的并且不再受虚拟机限制的世界。它和虚拟机拥有同样的权限。
4. 在 Hotspot JVM 中，直接将本地方栈和虚拟机栈合二为一
...
3. - 新生代中的 **Eden**:**From Survivor**:**To Survivor** 的比例是 **8:1:1**，可以通过`-XX:SurvivorRatio`来配置

4. 若在JDK 7中开启了 `-XX:+UseAdaptiveSizePolicy`，JVM 会动态调整 JVM 堆中各个区域的大小以及进入老年代的年龄
...
2. - 新生代又被进一步划分为**Eden区**和**Survivor区**，Survivor区由**From Survivor**和**To Survivor**组成

3. 当创建一个对象时，对象会被优先分配到新生代的Eden区

4. - 此时 JVM 会给对象定义一个**对象年轻计数器**（`-XX:MaxTenuringThreshold`）

5. 当 Eden 空间不足时，JVM 将执行新生代的垃圾回收（Minor GC）

6. - JVM 会把存活的对象转移到 Survivor 中，并且对象年龄 +1
   - 对象在 Survivor 中同样也会经历 Minor GC，每经历一次 Minor GC，对象年龄都会+1

7. 如果分配的对象超过了`-XX:PetenureSizeThreshold`，对象会**直接被分配到老年代**
